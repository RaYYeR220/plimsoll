...
| 5 | The canonical HCS records — live readings of real Base positions — have the shape their kind promises. The paid attestation's transfer exists. The refusal has no transfer | each record goes through the attestor's own `verify-charge` from the public topic. A record too large to carry its readings inline names a published evidence file in `manifest.json`, which `verify-charge` accepts only if it hashes to the digest the record anchors. The absence of a charge is then checked on the ledger (see below) |
