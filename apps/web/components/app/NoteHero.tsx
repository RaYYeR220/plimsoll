...
export function NoteHero({
  ...
  fundedBackingUsd?: number;
}) {
  const { demo } = useDemo();
  const state = stateForDemo(demo, recorded, obligationValue(obligation), { negativeControl, fundedBackingUsd });
  const family = state.family === 'evidence' ? 'evidence' : state.family === 'covered' ? 'covered' : 'asset';
  const showing =
    demo === 'recorded'
      ? demonstrated
      : negativeControl
        ? 'The negative control does not follow the demonstration. This is its funded backing, as read from its vault, against what it owes: it cannot clear, and that is the point of keeping it on the floor.'
